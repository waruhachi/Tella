...
while getopts ":v:i:o:n:x" args; do
    case "${args}" in
        v)
...
if [ -z "$output" ]; then
    ...
    new_filename=$(echo "$filename" | sed 's/\ /_/g' | sed 's/.ipa/_Patched.ipa/')
    output="$directory/$new_filename"
    echo "$info Setting output file as $output"
fi

# Ensure that the $directory variable is set
...
echo "$PWD/src/vendor/Azule/azule" -n "$output" -i "$ipa" -o "$directory" -f "$PWD/src/vendor/SatellaJailed/SatellaJailed.dylib" -muvwz | sed -u -r "s/(\[\*\])/$(echo $info)/g"
